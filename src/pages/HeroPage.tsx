    ...
    return (
        <h1 className={className} style={style}>
            {lines.map((line, lineIndex) => {
                const prevChars = lineLengths.slice(0, lineIndex).reduce((a, b) => a + b, 0);
                return (
                    <span key={lineIndex} className="block">
                        {line.split("").map((char, charIndex) => {
                            const delay = startDelay + (prevChars + charIndex) * charDelay;
                            return (
                                <AnimatedChar key={charIndex} delay={delay}>
                                    {char === " " ? "\u00A0" : char}
                                </AnimatedChar>
                            );
                        })}
                        ...
                        <div>
                            ...
                            <FadeIn delay={800} duration={1000} className="mb-5">
                                <p className="text-base md:text-lg text-gray-300 max-w-xl">
                                    Organize properties, manage rooms, track documents, and streamline
                                    real estate operations from one intelligent platform.
                                </p>
                            </FadeIn>
