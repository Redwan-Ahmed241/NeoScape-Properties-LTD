...
const DocumentCenter: React.FC<DocumentCenterProps> = ({ propertyName }) => {
  ...
  // Filtering states
  const [activeTab, setActiveTab] = useState<"all" | "property" | "room" | "tenant">("all");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const filteredRooms = allRooms.filter((r: any) => r.location === propertyName);
      setRooms(filteredRooms);

      const filteredAssignments = assignmentsData.filter((a: any) => a.property_name === propertyName);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let tenantId = form.linkType === "tenant" ? Number(form.tenantId) : undefined;
      let assignmentId = form.linkType === "tenant" ? Number(form.assignmentId) : undefined;
  ...
  const handleReviewSubmit = async (status: "approved" | "rejected") => {
    ...
    try {
      ...
      setDocuments(documents.map(d => d.id === reviewingDoc.id ? updated : d));
  ...
  const reminders = documents.filter((doc) => {
    ...
    return days > 0 && days <= (doc.reminderDays || 30) && doc.status === "active";
  });
  ...
  const inputCls = "h-10 w-full rounded-xl px-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-white/10 transition-all";
  ...
  if (!propertyName) {
    return (
      <div className="rounded-2xl py-14 text-center" style={{ background: "var(--surface-2)", border: "1px solid var(--border-default)" }}>
        ...
        <p className="text-white/40 text-sm">Select a property to view the Document Center.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border text-sm text-red-400" style={{ background: "rgba(239,68,68,0.05)", borderColor: "rgba(239,68,68,0.15)" }}>
          <AlertCircle className="w-4 h-4 flex-shrink-0" />
          <span className="flex-1">{error}</span>
          <button onClick={() => setError(null)} className="text-white/40 hover:text-white/80 transition-colors"><X className="w-4 h-4" /></button>
        </div>
      )}
      {success && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border text-sm text-emerald-400" style={{ background: "rgba(16,185,129,0.05)", borderColor: "rgba(16,185,129,0.15)" }}>
      ...
      {/* Reminders section */}
      {reminders.length > 0 && (
        <div className="rounded-xl px-4 py-3 border space-y-2" style={{ background: "rgba(245,158,11,0.04)", borderColor: "rgba(245,158,11,0.12)" }}>
          ...
          <div className="space-y-1">
            {reminders.map((doc) => (
              <p key={doc.id} className="text-xs text-white/60">
                Document <span className="font-semibold text-amber-300">{doc.name}</span> is expiring on {new Date(doc.expiryDate!).toLocaleDateString()}
              </p>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Property Level", value: documents.filter(d => !d.roomId && !d.tenantId).length, color: "text-blue-400" },
          { label: "Room Linked", value: documents.filter(d => d.roomId && !d.tenantId).length, color: "text-emerald-400" },
          { label: "Tenant Uploads", value: documents.filter(d => d.tenantId).length, color: "text-purple-400" }
        ].map(({ label, value, color }) => (
          <div key={label} className="rounded-xl p-4" style={{ background: "var(--surface-2)", border: "1px solid var(--border-default)" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/[0.05] pb-4">
        <div className="flex items-center gap-1.5 bg-white/5 p-1 rounded-xl shrink-0" style={{ border: "1px solid var(--border-subtle)" }}>
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeTab === t.id ? "bg-white text-black shadow-sm" : "text-white/50 hover:text-white hover:bg-white/[0.03]"
              }`}
            >
              {t.label}
            </button>
      ...
      {(isAdding || editingDoc) && (
        <div className="rounded-2xl p-5 space-y-4" style={{ background: "var(--surface-2)", border: "1px solid var(--border-default)" }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { setIsAdding(false); setEditingDoc(null); resetForm(); }} className="text-white/40 hover:text-white"><X className="w-4 h-4" /></button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Document Type</Label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Link Scope</Label>
              <div className="grid grid-cols-3 gap-2 bg-white/5 p-1 rounded-xl border border-white/[0.05]">
                {[
                  { id: "property", label: "Property", icon: <Home className="w-3 h-3" /> },
                  { id: "room", label: "Room", icon: <Home className="w-3 h-3 text-emerald-400" /> },
                  { id: "tenant", label: "Tenant", icon: <User className="w-3 h-3 text-purple-400" /> }
                ].map(opt => (
                  <button
                    key={opt.id}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, linkType: opt.id as any }))}
                    className={`flex items-center justify-center gap-1 py-1 rounded-lg text-xs font-medium transition-all ${
                      form.linkType === opt.id ? "bg-white text-black" : "text-white/55 hover:text-white"
                    }`}
                  >
                    {opt.icon}
                    {opt.label}
            ...
            <div>
              {form.linkType === "room" && (
                <>
                  <Label className="text-xs text-white/40 block mb-1.5">Select Room *</Label>
                  <select
                    className={inputCls} style={{ ...inputStyle, appearance: "none" }}
                    value={form.roomId}
                  ...
                  >
                    ...
                    {rooms.map(r => <option key={r.id} value={r.id}>{r.name} (${r.price}/mo)</option>)}
                  </select>
                </>
              )}
              {form.linkType === "tenant" && (
                <>
                  <Label className="text-xs text-white/40 block mb-1.5">Select Active Assignment *</Label>
                  <select
                    className={inputCls} style={{ ...inputStyle, appearance: "none" }}
              ...
              {form.linkType === "property" && (
                <div className="flex items-center gap-2 pt-6 text-xs text-white/35">
                  ...
                  <span>This document will apply to the entire property general scope.</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/[0.05] pt-4">
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">File URL (Optional if uploading file)</Label>
              <Input
                className={inputCls} style={inputStyle}
                value={form.fileUrl}
            ...
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Or Choose File from Device</Label>
              ...
              <p className="text-[10px] text-white/20 mt-1">Uploading auto-populates the File URL.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-white/[0.05] pt-4">
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Expiry Date</Label>
              <Input
            ...
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Renewal Date</Label>
              <Input
                type="date" className={inputCls} style={inputStyle}
            ...
            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Reminder Window (Days)</Label>
              <Input
          ...
          <div>
            <Label className="text-xs text-white/40 block mb-1.5">Additional Notes</Label>
            <Textarea
              rows={2}
              className="w-full rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-white/10 transition-all resize-none"
              style={inputStyle}
          ...
          <div className="flex gap-2 pt-2 border-t border-white/[0.05]">
            <button
              onClick={handleSave}
              disabled={uploading || !form.name || (!form.fileUrl && !uploadFile)}
      ...
      {reviewingDoc && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="rounded-2xl p-6 w-full max-w-md space-y-4 border border-white/10 shadow-2xl" style={{ background: "var(--surface-2)" }}>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-white">Review Tenant Document</h3>
              <button onClick={() => { setReviewingDoc(null); setReviewNotes(""); }} className="text-white/40 hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            
            <div className="bg-white/5 rounded-xl p-3 border border-white/[0.05] space-y-1 text-xs">
              <p className="text-white/40">Tenant Username: <span className="text-white font-medium">{reviewingDoc.tenantUsername}</span></p>
              <p className="text-white/40">Document Name: <span className="text-white font-medium">{reviewingDoc.name}</span></p>
              <p className="text-white/40">Description: <span className="text-white">{reviewingDoc.description || "No description provided."}</span></p>
              <p className="pt-2"><a href={reviewingDoc.fileUrl} target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:underline">Click to view/download file</a></p>
            </div>

            <div>
              <Label className="text-xs text-white/40 block mb-1.5">Review Notes / Admin Notes</Label>
              <Textarea
                rows={3}
                value={reviewNotes}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl py-14 text-center" style={{ background: "var(--surface-2)", border: "1px solid var(--border-default)" }}>
          <FileText className="w-10 h-10 text-white/10 mx-auto mb-3" />
          <p className="text-white/40 text-sm">No documents found matching this filter.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {filteredDocs.map((doc) => {
            const hasExpiry = !!doc.expiryDate;
            const daysLeft = hasExpiry ? Math.ceil((new Date(doc.expiryDate!).getTime() - Date.now()) / 86400000) : 0;
            const scopeLabel = doc.tenantId ? "Tenant Upload" : doc.roomId ? "Room Linked" : "Property Level";
              ...
              >
                <div className="flex items-start gap-3 min-w-0">
                  <div className="p-2.5 rounded-xl flex-shrink-0" style={{ background: "var(--surface-3)" }}>
                    {doc.tenantId ? <User className="w-4 h-4 text-purple-400" /> : <FileText className="w-4 h-4 text-white/40" />}
                  </div>
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <h4 className="text-sm font-semibold text-white truncate">{doc.name}</h4>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${getStatusStyle(doc.status)}`}>
                        {doc.status}
                      </span>
                    ...
                    <p className="text-xs text-white/30 flex flex-wrap gap-x-2">
                      <span>Type: <strong className="text-white/50 font-normal">{DOC_TYPES.find(t => t.value === doc.type)?.label || doc.type}</strong></span>
                      {doc.uploadDate && <span>Uploaded: <strong className="text-white/50 font-normal">{new Date(doc.uploadDate).toLocaleDateString()}</strong></span>}
                      {hasExpiry && (
                    ...
                    {doc.description && <p className="text-xs text-white/20 mt-1 max-w-xl line-clamp-1">{doc.description}</p>}
                    
                    {/* Extra contexts */}
                    <div className="flex flex-wrap items-center gap-2 mt-1.5 text-[10px] text-white/30">
                      {doc.roomId && <span>Room ID: <strong className="text-white/40 font-normal">{doc.roomId}</strong></span>}
                      {doc.tenantUsername && <span>Tenant: <strong className="text-white/40 font-normal">{doc.tenantUsername}</strong></span>}
                      {doc.uploadedBy && <span>Uploaded By: <strong className="text-white/40 font-normal">{doc.uploadedBy}</strong></span>}
                    </div>

                    {doc.adminNotes && (
                      <p className="text-xs text-purple-400/70 mt-1">Review Notes: {doc.adminNotes}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-1.5 self-end sm:self-center shrink-0">
                  {doc.status === "pending" && (
                    <button
                      onClick={() => { setReviewingDoc(doc); setReviewNotes(doc.adminNotes || ""); }}
